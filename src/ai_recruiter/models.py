...
class ResumeData(BaseModel):
    ...
    processing_notes: List[str] = Field(default_factory=list, description="Any issues during processing")


class ResumeAnalysis(BaseModel):
    """Analysis and insights about the resume."""
    resume_data: ResumeData
    strengths: List[str] = Field(default_factory=list, description="Top 3-5 professional strengths")
    suitable_roles: List[str] = Field(default_factory=list, description="Recommended job roles")
    skill_gaps: List[str] = Field(default_factory=list, description="Potential areas for improvement")
    overall_score: Optional[int] = Field(None, ge=1, le=10, description="Overall resume quality score (1-10)")
